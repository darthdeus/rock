...
pub fn declaration_pass(
    ...
    ast_walker::AstWalker::walk_module(module, &mut |ev| match ev {
        ...
        AstWalkEvent::OnEnter(AstNode::FunctionDeclaration(fndecl)) => {
            ...
            match scope_builder
                .table
                .lookup_symbol_in_scope_by_string(&fndecl.name.text, scope_builder.current_scope())
            {
            ...
            match &expr.kind {
                ast::ExprKind::Path(path) => {
                    ...
                    if path.idents.len() == 1 {
                        let _ =
                            scope_builder.register_identifier_symbol_usage(path.as_single_ident());
                    } else {
                ...
                ast::ExprKind::RefAlloc(_) => (),
                ast::ExprKind::Block(_) => (),
                ast::ExprKind::VecLiteral { .. } => (),
                ast::ExprKind::If { .. } => (),
                ast::ExprKind::StringInterpolation { .. } => (),
                ast::ExprKind::Index { .. } => (),
                ast::ExprKind::NullPtrLiteral => (),
